CREATE TABLE `admin` (
  ...
  `profile` text DEFAULT NULL COMMENT '备注',
...
CREATE TABLE `catalog` (
  ...
  `create_time` datetime DEFAULT NULL COMMENT '录入时间',
  `update_time`  timestamp NULL DEFAULT NULL COMMENT '更新时间' ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='目录' AUTO_INCREMENT=1;
...
CREATE TABLE `show` (
  ...
  `update_time` timestamp NULL DEFAULT NULL COMMENT '更新时间' ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=INNODB DEFAULT CHARSET=utf8 COMMENT='内容页';
...
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='设置' AUTO_INCREMENT=1 ;
